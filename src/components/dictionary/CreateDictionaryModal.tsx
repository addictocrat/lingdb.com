  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast(data.error || t('error_generic'), 'error');
        return;
      }
      ...
      toast(t_common('errors.generic'), 'error');
    } finally {
    ...
    <Modal isOpen={isOpen} onClose={onClose} title={t('title')} size="md">
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-lg font-medium">{t('field_description')}</label>
          <textarea
            value={description}
        ...
        <div className="space-y-1.5">
          ...
          <div className="grid grid-cols-5 gap-2">
            {languages.map((lang) => (
              ...
              >
                <span className={`text-2xl rounded-sm overflow-hidden ${lang.flagClass}`}></span>
                <span className="font-medium">{t_settings(`languages.${lang.code}`)}</span>
              </button>
            ))}
          </div>
        </div>
