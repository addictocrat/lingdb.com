    ...
    >
      ...
      <div className="relative mb-4 flex items-center justify-between">
        <div className="flex items-center gap-2 rounded-full bg-[var(--bg)] px-3 py-1 text-xs font-bold text-primary-600">
          ...
          {new Date(blog.createdAt).toLocaleDateString(locale, { month: 'short', day: 'numeric', year: 'numeric' })}
        </div>
        {blog.keywords && (
          <div className="flex gap-1">
            {blog.keywords.split(',').slice(0, 2).map(kw => (
              <span key={kw} className="rounded-full bg-primary-500/10 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-primary-500">
                {kw.trim()}
              </span>
            ))}
          </div>
      ...
      <div className="mt-6 flex items-center justify-between border-t border-[var(--border-color)] pt-4">
        <div className="flex items-center gap-2 text-sm text-[var(--fg)]/40">
          ...
          <span className="font-semibold">{blog.author?.username || 'Lingdb Team'}</span>
        </div>
        <Link href={`/${locale}/blogs/${blog.slug}`} className="flex items-center gap-1 font-bold text-primary-500 transition-all hover:gap-2">
          Read More
