...
export default function QuizQuestion({ question, onAnswer }: QuizQuestionProps) {
  ...
  const handleChoiceClick = (option: string, buttonRef: HTMLButtonElement | null) => {
    ...
    if (buttonRef) {
      if (isCorrect) {
        gsap.to(buttonRef, { backgroundColor: 'rgb(34, 197, 94)', color: 'white', duration: 0.2 });
      } else {
        gsap.to(buttonRef, { backgroundColor: 'rgb(239, 68, 68)', color: 'white', duration: 0.2 });
        gsap.to(buttonRef, { keyframes: [{x:-10}, {x:10}, {x:-10}, {x:10}, {x:0}], duration: 0.4, ease: 'power2.out' });
      }
    }
  ...
  const handleTypeSubmit = (e: React.FormEvent) => {
    ...
    const isCorrect = cleanAnswer === cleanCorrect || cleanCorrect.includes(cleanAnswer) && cleanAnswer.length > 3;
    ...
    if (inputArea) {
      if (isCorrect) {
        ...
        gsap.to(inputArea, { keyframes: [{x:-10}, {x:10}, {x:-10}, {x:10}, {x:0}], duration: 0.4, ease: 'power2.out' });
      }
    }
  ...
  return (
    <div ref={containerRef} className="w-full max-w-xl mx-auto flex flex-col items-center p-6 bg-[var(--surface)] border border-[var(--border-color)] rounded-2xl shadow-sm">
      ...
      {(question.type === 'multiple_choice' || question.type === 'reverse_choice') && question.options && (
      ...
      {question.type === 'type_answer' && (
        <form onSubmit={handleTypeSubmit} className="w-full relative">
          <div id="type-answer-area" className="w-full rounded-xl border-2 border-[var(--border-color)] p-2 transition-colors">
            <input
          ...
          {hasAnswered && (
             <div className={`mt-6 p-4 rounded-xl flex items-center gap-3 w-full justify-center ${
               typedAnswer.trim().toLowerCase() === question.correctAnswer.toLowerCase() || question.correctAnswer.toLowerCase().includes(typedAnswer.trim().toLowerCase()) && typedAnswer.trim().toLowerCase().length > 3
               ...
               {typedAnswer.trim().toLowerCase() === question.correctAnswer.toLowerCase() || question.correctAnswer.toLowerCase().includes(typedAnswer.trim().toLowerCase()) && typedAnswer.trim().toLowerCase().length > 3 ? (
                 ...
                 <div className="text-center w-full">
                   ...
                   <p className="text-lg opacity-90 w-full whitespace-normal">Correct answer: <strong>{question.correctAnswer}</strong></p>
                 </div>
               )}
             </div>
          )}
