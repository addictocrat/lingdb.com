...
export default function BlogEditor({
  ...
  const saveBlogMutation = useMutation({
    mutationFn: (payload: Record<string, unknown>) =>
      saveAdminBlog(blog?.id ?? null, payload),
    onSuccess: async () => {
  ...
  const generateBlogMutation = useMutation({
    mutationFn: (promptValue: string) => generateAdminBlog(promptValue),
  });

  const translateBlogMutation = useMutation({
    mutationFn: (blogId: string) => translateAdminBlog(blogId),
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: ["admin", "blogs"] });
    },
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast(getErrorMessage(error, "Failed to save"), "error");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      const data = await generateBlogMutation.mutateAsync(prompt);
      setFormData({
        ...formData,
  ...
  const handleTranslate = async () => {
    ...
    try {
      const data = await translateBlogMutation.mutateAsync(blog.id);
      ...
      setTranslations((prev) => ({
        ...
        ...Object.fromEntries(
          Object.entries(
            (data.translations as Record<string, Partial<TranslationData>>) ||
              {},
          ).map(([loc, t]) => [
            loc,
            {
      ...
      toast(getErrorMessage(error, "Translation failed"), "error");
    } finally {
