...
import {
  deleteModeratedDictionary,
  updateModeratedDictionaryVisibility,
...
export default function DictionaryModerationClient({
  initialDictionaries,
}: {
  initialDictionaries: ModerationDictionary[];
}) {
  ...
  const deleteDictionaryMutation = useMutation({
    mutationFn: deleteModeratedDictionary,
    onSuccess: async () => {
  ...
  const updateVisibilityMutation = useMutation({
    mutationFn: ({ id, isPublic }: { id: string; isPublic: boolean }) =>
      updateModeratedDictionaryVisibility(id, isPublic),
    onSuccess: async () => {
      await queryClient.invalidateQueries({
        queryKey: ["admin", "dictionaries"],
      });
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm(
        "Are you sure you want to delete this dictionary and all its words?",
      )
    )
      return;

    try {
      await deleteDictionaryMutation.mutateAsync(id);
  ...
  const togglePublic = async (dict: ModerationDictionary) => {
    ...
    try {
      await updateVisibilityMutation.mutateAsync({ id: dict.id, isPublic });
          ...
          <tbody className="divide-y divide-[var(--border-color)]">
            {filteredDicts.map((dict) => (
              <>
                <tr
                  key={dict.id}
                  className={`transition-colors hover:bg-[var(--bg)]/30 ${seoEditingId === dict.id ? "bg-amber-500/5" : ""}`}
                >
                  <td className="px-6 py-4">
                    <div>
                      ...
                      <p className="text-sm uppercase text-[var(--fg)]/40 tracking-wider font-bold">
                        {dict.language}
                      </p>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex justify-end gap-2">
                      <button
                        onClick={() =>
                          setSeoEditingId(
                            seoEditingId === dict.id ? null : dict.id,
                          )
                        }
                  ...
                  <tr key={`${dict.id}-seo`} className="bg-amber-500/5">
                    <td colSpan={5} className="px-6 pb-6 pt-0">
                      <AdminSeoEditor
                        ...
                        seoGeneratedAt={
                          dict.seoGeneratedAt?.toISOString() ||
                          (dict.seoGeneratedAt as unknown as string) ||
                          null
                        }
                      />
                    </td>
