...
export async function PATCH(request: Request) {
  try {
    ...
    const { username, locale, hasCompletedTour, hasCompletedDictTour } =
      result.data;

    // Double check username availability if it's being changed
    if (username) {
      const existingUser = await db.query.users.findFirst({
        where: and(eq(users.username, username), ne(users.supabaseId, user.id)),
      });
    ...
    const updateData: {
      updatedAt: Date;
      username?: string;
      locale?: "en" | "fr" | "de" | "es" | "tr";
      hasCompletedTour?: boolean;
      hasCompletedDictTour?: boolean;
    } = { updatedAt: new Date() };
    if (username) updateData.username = username;
    if (locale) updateData.locale = locale;
    if (typeof hasCompletedTour === "boolean")
      updateData.hasCompletedTour = hasCompletedTour;
    if (typeof hasCompletedDictTour === "boolean")
      updateData.hasCompletedDictTour = hasCompletedDictTour;
