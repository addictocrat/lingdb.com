...
export default async function LibraryPage({
  ...
  const lang =
    typeof resolvedParams.lang === "string" ? resolvedParams.lang : "";
  const sort =
  ...
  const wordCountQuery = sql<number>`count(distinct ${words.id})`.mapWith(
    Number,
  );
  const forkCountQuery = sql<number>`count(distinct ${forks.id})`.mapWith(
    Number,
  );
  ...
  if (sort === "most_forked")
    sortOrder = desc(sql`count(distinct ${forks.id})`);
  ...
  const t = await import("next-intl/server").then((m) =>
    m.getTranslations("library"),
  );
  const tCommon = await import("next-intl/server").then((m) =>
    m.getTranslations("common"),
  );
  ...
  queryClient.setQueryData(
    qk.dictionaries.list({ q, lang, sort, page }),
    libraryItems,
  );
